...
namespace iRLeagueDatabaseCore.Models
{
    public class StandingRowEntity
    {
        ...
        {
            ResultRows = new HashSet<ScoredResultRowEntity>();
        }
        ...
        public virtual ICollection<ScoredResultRowEntity> ResultRows { get; set; }
    }
    ...
    public class StandingRowEntityConfiguration : IEntityTypeConfiguration<StandingRowEntity>
    {
        public void Configure(EntityTypeBuilder<StandingRowEntity> entity)
        {
            ...
            entity.HasMany(p => p.ResultRows)
                .WithMany(d => d.StandingRows)
                .UsingEntity<StandingRows_ScoredResultRows>(
                    right => right.HasOne(e => e.ScoredResultRow)
                        .WithMany()
                        .HasForeignKey(e => new { e.LeagueId, e.ScoredResultRowRefId }),
                    left => left.HasOne(e => e.StandingRow)
                        .WithMany()
                        .HasForeignKey(e => new { e.LeagueId, e.StandingRowRefId }));
        }
    }
}
